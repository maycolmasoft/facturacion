...
class Conectar{
    ...
    public function conexion(){
        ...
        if($this->driver=="pgsql" || $this->driver==null){
        	...
        	$con = pg_connect("host=localhost port=5432 dbname=facturacion user=postgres password=Clave1*");
        	if(!$con){
    ...
    public function startFluent(){
        ...
        if($this->driver=="pgsql" || $this->driver==null){
        	...
        	try
        	{
        		$pdo = new PDO('pgsql:host=localhost;port=5432;dbname=facturacion', 'postgres', 'Clave1*' );
